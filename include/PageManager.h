...
public:
	...
	//// 新增：位图相关常量
    static const unsigned int BITS_PER_WORD = 32;  // 每个unsigned int包含的位数
    static const unsigned int MAX_PAGES = 8192;     // 最大页面数，根据实际物理内存大小调整
...
private:
	...
	void ClearBit(unsigned int bitIndex);
	bool TestBit(unsigned int bitIndex);
	int FindContiguousPages(unsigned int numPages);
...
public:
	...
	//位图(0空闲，1已分配)
	unsigned int m_bitmap[MAX_PAGES / BITS_PER_WORD];
	unsigned int m_startPage;  // 起始页号
	unsigned int m_totalPages; // 总页数
